...
import com.oracle.coherence.examples.sockshop.spring.orders.controller.OrderController;
...
public class CoherenceOrderResourceIT {
    ...
    protected void testCreateOrder() {
        ...
        Eventually.assertDeferred(() -> orders.get(orderId).getStatus(), is(Order.Status.SHIPPED));
    ...
    protected void testPaymentFailure() {
        ...
        final String orderId = orders.getLastOrderId();
        Eventually.assertDeferred(() -> orders.get(orderId).getStatus(), is(Order.Status.PAYMENT_FAILED));
